...
const AuthLayout = ({ children }) => {
  return <div className='flex'>
    ...
    <div className='hidden md:block w-[40vw] h-screen bg-violet-50 bg-auth-bg bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative'>
        ...
        <div className='w-48 h-56 rounded-[40px] border-[20px] border-fuchsia-600 absolute top-[30%] -right'/>
...
const StatsInfoCard = ({ icon, label, value, color }) => {
  return (
    <div className="flex gap-6 bg-white p-4 rounded-xl shadow-md shadow-purple-400/10 border border-gray-200/50 z-10">
      <div className={'w-12 h-12 flex items-center justify-center text-[26px] text-white ${color} rounded-full dwop-shadow-xl'}>
        {icon}
      </div>
      <div>
        <h6 className='text-xs text-gray-500 mb-1'>{label}</h6>
        <span className='test-[20px]'>${value}</span>
      </div>
    </div>
  );
}
